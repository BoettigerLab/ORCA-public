...
% Last Modified by GUIDE v2.5 29-Jul-2017 12:27:16
...
defaults(end+1,:) = {'dataTypes','cell',{'H','T','R','E'}}; % hybe toe repeat expression
defaults(end+1,:) = {'alignmentBoxWidth', 'positive', 256}; % pixels.  size of region to use for frame alignment
...
parsFit = ParseVariableArguments([],defaults,'ChrTracer_FitSpots');
% Link and Render -------------------------------------------------%
defaults = cell(0,3);
...
elseif strcmp(currStepName,'Save Aligned Data') % 'Adjust Alignment'
    ...
    [CT{handles.id}.fidMapData,CT{handles.id}.datMapData,...
     CT{handles.id}.fiducialFrames,CT{handles.id}.dataFrames] = ...
        ChrTracer2_SaveAlignedData(pars.eTable,CT{handles.id}.regData,'parameters',pars);
%---------------------------- SELECT ROI ---------------------------------%
elseif strcmp(currStepName,'Select ROI') % SELECT ROI
    
    if CT{handles.id}.parsROI.useFid
        im = CT{handles.id}.fiducialFrames;
    else
...
elseif strcmp(currStepName,'Crop Spots') % PLOT SPOTS
     ...
     pars.currentSpot = CT{handles.id}.currentSpotID;
     pars.saveData = false; 
...
CT{handles.id}.fiducialFrames = dataOut.fiducialFrames;
CT{handles.id}.regData = dataOut.regData;
% CT{handles.id}.regData = globalRegData;
% update some of the parameters
CT{handles.id}.parsFOV.goodHybes = dataOut.goodHybes;
CT{handles.id}.parsFOV.eTable = dataOut.eTable;
CT{handles.id}.parsFOV.dataFolder = dataOut.dataFolder;
...
if length(daxFiles1) >= length(daxFiles2)
    daxFiles = daxFiles1;
else
    daxFiles = daxFiles2;
end
