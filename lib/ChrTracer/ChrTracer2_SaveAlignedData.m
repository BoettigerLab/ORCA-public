...
fidNames = cellstr(ls([pars.saveFolder,'fov',num2str(pars.fov,'%03d'),'_h*fid.dax']));
datNames = cell(numHybes,numDataChns);
for n=1:numDataChns
    foundDat = cellstr(ls([pars.saveFolder,'fov',num2str(pars.fov,'%03d'),'_h*data',num2str(n),'.dax']));
    if isempty(foundDat{1})
        foundDat = cellstr(ls([pars.saveFolder,'fov',num2str(pars.fov,'%03d'),'_h*dat',num2str(n),'.dax']));
    end
...
if ~skip
    ...
    for h=allHybes
        ...
        fidInfName = ['fov',num2str(pars.fov,'%03d'),'_h',num2str(h,'%03d'),'_fid','.inf'];
        if ( exist([pars.saveFolder,fidInfName],'file')==0 || pars.overwrite ) || h==pars.refHybe
            ...
            for n = 1:numDataChns
                ...
                datInfo.localName = ['fov',num2str(pars.fov,'%03d'),'_h',num2str(h,'%03d'),'_data',num2str(n),'.inf']; 
            ...
            for n = 1:numDataChns
                localName = ['fov',num2str(pars.fov,'%03d'),'_h',num2str(h,'%03d'),'_data',num2str(n),'.inf']; 
