function [imageTiles,stageXY] = LoadRegDaxAsMosaic(maxNameInHybFov,varargin)
% Inputs:
...
for h=chns
    ...
    if h==chns(1) || isempty(regDatas{1}) || h>length(regDatas{1}) % Load the first hyb
        [chnTiles,stageXY] = DaxToImageTiles(imTiles(:,h),'parameters',pars);
        % stageXY = stageXY + ones(2,numFovs)*pars.trim;
    ...
    chnTiles = FlattenBackground(chnTiles,'parameters',pars);
    % save data
