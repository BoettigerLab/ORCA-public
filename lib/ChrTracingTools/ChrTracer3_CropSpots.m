function [fidSpots,dataSpots,pars] = ChrTracer3_CropSpots(fidMapData,datMapData,varargin)
...
defaults(end+1,:) = {'eTable','table',[]};
defaults(end+1,:) = {'lociXY','array',[]};
% key parameters
...
t = toc;
disp(['spot cropped in ',num2str(t),' s']);
...
if pars.showPlots
    ...
    for s=pars.currentSpot
        if hasFid
            ...
            tileFigFid = figure(4); clf; 
                ...
                TileImage(IncreaseContrast(im),'tileLabels',tileLabels_fid,'showImage',true);  
        ...
        if hasDat
            ...
            tileFigDat = figure(6);  clf;
                ...
                TileImage(IncreaseContrast(im),'tileLabels',tileLabels_dat,'showImage',true);
    ...
    t = toc;
    disp(['data plotted in ',num2str(t),' s']);
end