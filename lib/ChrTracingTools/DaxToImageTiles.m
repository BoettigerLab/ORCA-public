...
for f = 1:numIms
    if pars.projectAll 
        ...
        if exist(maxFile,'file')~=0 && pars.loadProjections  % use previously written
            [dax,infoFile] = ReadDax(maxFile,'verbose',pars.verbose);
        elseif pars.loadInfoOnly
            infoFile = ReadInfoFile(maxFile,'verbose',pars.verbose);
            dax = [];
        else
            [dax,infoFile] = ReadDax(daxFiles{f},'verbose',pars.verbose);
            dax = max(dax,[],3); 
        ...
        [dax,infoFile] = ReadDax(daxFiles{f},'startFrame',pars.selectFrame,'endFrame',pars.selectFrame,'verbose',pars.verbose);
    end   
