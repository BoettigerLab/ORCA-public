function spotDataTable = ChrTracer2_FitAllSpots(fidSpots,datSpots,varargin)
...
if pars.numParallel < 2
    for s = selectSpots % 320;
        ...
        dataTable{s} = ChrTracer2_FitSpots(fidSpts,datSpts,s,'parameters',pars,'veryverbose',true);
    end
else
    parfor s=selectSpots
        fidSpts = fidSpots{s};
        datSpts = datSpots{s,1};
        dataTable{s} = ChrTracer2_FitSpots(fidSpts,datSpts,s,'parameters',pars);
    end
end
