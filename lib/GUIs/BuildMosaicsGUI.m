classdef BuildMosaicsGUI < StepGUIclass
% Update Log
    ...
    methods     
        ...
        function self = AlignHybs(self,pars)
            ...
            if nHybs > 1
                ...
                disp('no need to align single hyb, advance to next step');
            end
        end
        % ---- Validate Mosaic
        function self = ValidateMosaic(self,pars)
            [self.imageTiles,self.stageXY] = LoadRegDaxAsMosaic(self.maxNameInHybFov,'parameters',pars);
            % display results
            ...
            self.stepParameters{5}.transpose = pars.transpose;
            self.stepParameters{5}.fliplr = pars.fliplr;
            self.stepParameters{5}.flipud = pars.flipud;
            self.stepParameters{5}.backgroundCorrect = pars.backgroundCorrect;
            % save parameters
            savePars = struct2table(pars);
...
%             end
        ...
        function self = SaveTif(self,pars)
            ...
            if ~skip
                ...
                % write table of uls
                ulTable = [outputFolder,saveRoot,'tileULs.csv'];
