...
% MosaicBuilder specific dynamic properties
    ...
    properties % (Access = private) % 
       allStepNames = {'Load Mosaic1',...
                       ...
                       'Save Mosaic Data',...
                       'Explore Mosaic'};
       stepDirections = {'Step 1: load mosaic 1 (typically DNA/ORCA data).  Validate that FOVs array properly into the mosaic. Change flipud, fliplr, and transpose if FOV tiles are flipped.';
    ...
    methods            
        ...
        function self = LoadMosaic1(self,pars)
            ...
            self.analysisFolder1 = saveName;
            % --- interpet channel names
            if pars.hybs == 1 && strcmp(pars.dataType,'fiducial')
                self.chnNames1 = {'fiducial1'}; % default load condition
            else
                [~,self.chnNames1] = TileLabelsFromEtable(eTable1,'style','names');      % Record Names
                % otherwise take names from table
            end 
            % load names of all data requested
            [~,~,~,daxNames1] =  LoadDaxFromEtable(eTable1_xls,...
                            'parameters',pars,'saveFolder',self.analysisFolder1,...
                            'hybNumber',pars.hybs,'fov',pars.fovs);
            % create hybe-registered image tiles of all data
            [self.imageTiles1,stageXY1] =LoadRegDaxAsMosaic(daxNames1,...        % Image tiles in memory
                'parameters',pars,'saveFolder',self.analysisFolder1);
            xyShifts1 = readtable([self.analysisFolder1,'regFOV.csv']);     % 
            xyShifts1 = [xyShifts1.xShift,xyShifts1.yShift]; % convert table to matrix  
            self.uls1 = stageXY1 + xyShifts1;                                % Corrected image positions in memory
            mos1 = MosaicViewerRender(self.imageTiles1,self.uls1,...
        ...
        function self = LoadMosaic2(self,pars)
            ...
            self.analysisFolder2 = saveName;
            % get channel names from experiment table
            [~,chnNames] = TileLabelsFromEtable(eTable2,'style','names');
            self.chnNames2 = cat(1,chnNames,'fiducial2');
            % load names of all data requested
            [~,~,~,daxNames2] =  LoadDaxFromEtable(eTable2_xls,...
                            'parameters',pars,'saveFolder',self.analysisFolder2,...
                            'hybNumber',pars.hybs,'fov',pars.fovs);
            % create hybe-registered image tiles of all data
            [self.imageTiles2,stageXY2] =LoadRegDaxAsMosaic(daxNames2,...
                'parameters',pars,'saveFolder',self.analysisFolder2);
            % fetch xy-shifts from CSV if not in memory
            xyShifts2 = readtable([self.analysisFolder2,'regFOV.csv']);
            xyShifts2 = [xyShifts2.xShift,xyShifts2.yShift]; % convert table to matrix  NO LONGER NEEDED 
            self.uls2 = stageXY2 + xyShifts2;
            % plot the mosaic
              mos2 = MosaicViewerRender(self.imageTiles2(:,1),self.uls2,...
                'parameters',pars,'downsample',4,...
                'fliplr',false,'flipud',false,'transpose',false);            
        ...
        function CoarseAlign(self,pars)
            ...
            coarseAlignFile = [self.analysisFolder2,'coarseAlign.csv'];
            answer = 'Overwrite';
            ...
            switch answer
                ...
                case 'Load'
                    ...
                    self.coarseAlign = table2struct(coarseAlignTable);
                    if isfield(self.coarseAlign,'center_1') % convert center_1,center_2 back to array
                        self.coarseAlign.center = [self.coarseAlign.center_1,self.coarseAlign.center_2];
                    end
                case 'Overwrite'
                    % ---- Create downsampled mosaics
                    mos1 = TilesToMosaic(self.imageTiles1,self.uls1,'padMosaic',1,'parameters',pars);
                    mos2 = TilesToMosaic(self.imageTiles2(:,1),self.uls2,'padMosaic',1,'parameters',pars);                    
                    % --- Merge and contrast
                    imO = MergeImages(mos1,mos2,'align','matchFirst'); % pad appropriately to make same size. 
                ...
                %     figure(1); clf; % for troubleshooting
        ...
        function FineAlign(self,pars)
            ...
            [numFOV2, numChns2] = size(self.imageTiles2);           
            fovToDock = 1:numFOV2;
            tableName = [self.analysisFolder2,'uls2to1.csv'];
            ...
            theta = self.coarseAlign.theta;
            disp('applying rotation to imTiles for Mosaic2');
            [uls2_align,imTiles2] = RotateTranslatePoints(self.uls2,self.coarseAlign,...
                                'center',self.coarseAlign.center,...
                                'imageTiles',self.imageTiles2,...
                                'invert',false,...
                                'opOrder','TR');
            disp('creating full resolution image for Mosaic1');
            [self.mosaic1,self.refULs] = TilesToMosaic(self.imageTiles1,self.uls1,'padMosaic',1);
            % figure(11); clf; imagesc(IncreaseContrast(self.mosaic1,'high',.99999));
            % figure(12); clf; imagesc(IncreaseContrast(imTiles2{1,end},'high',.999));
            ...
            for f = fovToDock
                ...
                [alignValues{f},~,valid] = InteractiveAlign(self.mosaic1,imTiles2{f,end},... 
                            'parameters',pars,...
        ...
        function ValidateAlign(self,pars)
            ...
            for n=1:numFOV2 
                im = self.imageTiles2{n,numChns2};
            ...
            mosaicDNA = IncreaseContrast(self.mosaic1,'high',pars.redContrastHigh,'low',pars.redContrastLow);
        ...
        function AssembleMosaic(self,pars)
           ...
           self.comboMosaic = cat(3,self.mosaic1,mosaicStack{:}); 
           chnName = cat(1,self.chnNames1,self.chnNames2);
           NcolorApp(self.comboMosaic,'names',chnName);
        ...
        function SaveAlignment(self,pars) % SaveAlignment
            ...
            if ~skip
                ...
                writetable(table(x,y,w,h,trim,fliplr,flipud,transpose),ulTable);
                try
                for c=1:nChns
                  	mosaicOut= self.comboMosaic(:,:,c); 
