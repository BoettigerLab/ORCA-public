...
[symbol,logx,logy,hhx,hhy,clr]=getproperties(varargin);
...
if ~isempty(xl) % x errorbars
    ...
    h = plotfct(barx,bary,esymbol,'color',clr); hold on
end
if ~isempty(yl) % y errorbars
    [barx,bary]=barline(x,yl,yh,logx,hhy); 
    h = [plotfct(barx,bary,esymbol,'color',clr);h]; hold on
end
if ~isempty(y) % function values
    h = [plotfct(x,y,symbol,'color',clr);h];
end
...
function [sym,lx,ly,hx,hy,clr] = getproperties(A)
lx=0; ly=0; hx=2/3; hy=2/3; sym='-'; clr = 'b'; % presets
