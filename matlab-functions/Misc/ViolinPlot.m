        ...
        switch floor(opt.histOpt)
            ...
            case 1
                ...
                if opt.histOpt == 1.1
                    ...
                    [dummy,x] = histogramDP(currentData); % updated to not overwrite matlab's intrinsic histogram fxn
                    if length(x) == 1
            ...
            case 2
                ...
                [xHist,yHist] = histogramDP(currentData,opt.divFactor,0); % updated to not overwrite matlab's
            case 99
    ...
    switch opt.xyOri
        case 'normal'
            if any(opt.showMM==[1,2])
                mh = plot(ah,opt.xValues+xOffset,m,'k+','Color','k','MarkerSize',markerSize);
            end
            if any(opt.showMM==[1,3])
                mdh = plot(ah,opt.xValues+xOffset,md,'k.','MarkerSize',markerSize);
            end
            if opt.showMM == 4
                mh = plot(ah,opt.xValues+xOffset,m,'k+','Color','k','MarkerSize',markerSize);
        ...
        case 'flipped'
            if any(opt.showMM==[1,2])
                mh = plot(ah,m,opt.xValues+xOffset,'k+','Color','k','MarkerSize',markerSize);
            end
            if any(opt.showMM==[1,3])
                mdh = plot(ah,md,opt.xValues+xOffset,'k.','MarkerSize',markerSize);
            end
            if opt.showMM == 4
                mh = plot(ah,m,opt.xValues+xOffset,'k+','Color','k','MarkerSize',markerSize);
