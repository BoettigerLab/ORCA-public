function [daxFinalName,fidName,daxNoNormName,parameters] = SaveCellImagesByBit(movieN,infoPerCell,movieFid,newDataPath,varargin)
...
for c=1:numCells 
    ...
    if (~(file1Exists && file2Exists && file3Exists) || parameters.overwriteDax) && notEmpty    
        ...
        infoB = infoN; % info file for raw data file
        infoB.localName = regexprep(daxNoNormName{c},'.dax','.inf'); 
        WriteDAXFiles(daxOutNoNorm,infoB,'verbose',false); 
        
        if parameters.laplaceFilter
            daxOutLaplace = squeeze(movieNWL(:,:,c,:)); 
            infoB = infoN; % info file for raw data file
            infoB.localName = regexprep(daxLaplaceName{c},'.dax','.inf'); 
            WriteDAXFiles(daxOutLaplace,infoB,'verbose',false); 
        end
        
        if parameters.deconvolve
            daxOutNormalized = squeeze(movieNWLD(:,:,c,:)); 
            infoB = infoN; % info file for warped data  
            [yDim,xDim,~] = size(daxOutNormalized);
            infoB.frame_dimensions = [xDim,yDim];
            infoB.frame_size = infoB.frame_dimensions(1)*infoB.frame_dimensions(2);
            infoB.notes = ['upsampled by ',num2str(parameters.upsample)];
            infoB.localName = regexprep(daxFinalName{c},'.dax','.inf'); 
            WriteDAXFiles(daxOutNormalized,infoB,'verbose',false); 
        end
        
        infoF = infoN; % info file for fiducial dax
        infoF.number_of_frames = 1;
        infoF.localName = regexprep(fidName{c},'.dax','.inf'); 
        fiducialDax = squeeze(movieFidwarped(:,:,c,:));
        WriteDAXFiles(fiducialDax,infoF,'verbose',false);              
    else
        if parameters.verbose
