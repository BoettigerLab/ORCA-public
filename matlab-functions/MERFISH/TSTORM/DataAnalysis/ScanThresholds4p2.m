...
while ~isDone
    ...
    for c=1:length(parameters.cellsToAnalyze)
        ...
        % -------------------------------------------------------------------------
        mLists = [mListsByCell{:}];
        [wordsDetected,~,brightnessPerSpotPerCell{c}] = FindWords(mListsByCell, [cat(1,mLists.xc), cat(1,mLists.yc)], ...
    ...
    brightnessPerSpot = cat(1,brightnessPerSpotPerCell{:});
    % -------------------------------------------------------------------------
    % Save Words
    % -------------------------------------------------------------------------
    ...
    % -------------------------------------------------------------------------
    objective(iterationNumber) = parameters.objectiveFunction(localWords,brightnessPerSpot);
