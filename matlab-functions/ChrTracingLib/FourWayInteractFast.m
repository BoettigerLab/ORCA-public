function  [probChange,numObs,typeM] = FourWayInteractFast(distMap,varargin)
...
probChange = nan(npts,npts,npts,npts);
numObs = nan(npts,npts,npts,npts);
% distAcB =  zeros(npts,npts,npts,npts);
% distAnB =  zeros(npts,npts,npts,npts);
typeM = numObs;
tic
parfor ai=1:npts  % can parfor
    for bi=1:npts  %   ENFORCED, without loss of generality, a<b, c<d  
        for ci=1:npts
            for di=1:npts
                a = pts(ai); 
                b = pts(bi);
                ...
                if (a<b && c<d) && ~(a == c && b== d)
                    ...
                    elseif isSeparate && c>b    
                        extra =  squeeze(contactMap(b,d,hasData)) ;    
                    elseif isThreeWay % literally only 3 points  
                        extra = [];
