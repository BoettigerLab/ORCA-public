function [z,mu,sigma] = NanZscore(x,varargin)
...
%   constant at 0).
%
%   [Z,MU,SIGMA] = NanZscore(X) also returns MEAN(X) in MU and STD(X) in SIGMA.
%
%   [...] = NanZscore(X,1) normalizes X using STD(X,1), i.e., by computing the
...
%   ZSCORE(X).
%
%   [...] = NanZscore(X,FLAG,'all') standardizes X by working on all the
...
%   DIM of X.
%
%   [...] = NanZscore(X,FLAG,VECDIM) standardizes X by working along the all the
