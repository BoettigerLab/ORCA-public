function varargout = ChrTracer3p0(varargin)
% CHRTRACER3P0 MATLAB code for ChrTracer3p0.fig
%      CHRTRACER3P0, by itself, creates a new CHRTRACER3P0 or raises the existing
%      singleton*.
%
%      H = CHRTRACER3P0 returns the handle to a new CHRTRACER3P0 or the handle to
%      the existing singleton*.
%
%      CHRTRACER3P0('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CHRTRACER3P0.M with the given input arguments.
%
%      CHRTRACER3P0('Property','Value',...) creates a new CHRTRACER3P0 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before ChrTracer3p0_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to ChrTracer3p0_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help ChrTracer3p0

% Last Modified by GUIDE v2.5 02-Apr-2019 09:03:09

% Begin initialization code - DO NOT EDIT
gui_Singleton = 0;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @ChrTracer3p0_OpeningFcn, ...
                   'gui_OutputFcn',  @ChrTracer3p0_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before ChrTracer3p0 is made visible.
function ChrTracer3p0_OpeningFcn(hObject, eventdata, handles, varargin) %#ok<*INUSL>
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to ChrTracer3p0 (see VARARGIN)

% all data for the chromosome tracer is kept inside a structure array;
% so there can be multiple version of ChrTracer3p0 open, each version has its
...
% Choose default command line output for ChrTracer3p0
...
% UIWAIT makes ChrTracer3p0 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = ChrTracer3p0_OutputFcn(hObject, eventdata, handles) 
global CT;
...
% defaults(end+1,:) = {'alignmentBoxWidth', 'positive', 800}; % pixels.  size of region to use for frame alignment  % alignmentBoxWidth
...
defaults(end+1,:) = {'alignMaxProject','boolean',false}; % save maximum projection for quick loading in future
defaults(end+1,:) = {'saveMaxProject','boolean',true}; % save maximum projection for quick loading in future
% alignment parameters
defaults(end+1,:) = {'maxSize', 'positive', 500}; % rescale all images to this size for alignment
defaults(end+1,:) = {'fineCenter','array',[0,0]};
defaults(end+1,:) = {'gradMax', 'boolean', true}; % gradient of auto correlation or peak of autocorrelation 
...
CT{handles.id}.stepNames = {'Load Ex. Table';
                            'Save FOV Overlay';  % Save FOV Overlay
                            'Save Aligned Data';
                            'Select ROI';
                            'Crop Spots';
                            'Fit Spots'};
...
'Select "Save FOV Overlay" to see results of x,y drift correction.  If image looks white instead of color shifted, proceed to "Next Step".';
...
if strcmp(currStepName,'Load Ex. Table')
    ...
    if choice 
        ...
        dataOut = ChrTracer3_LoadData(CT{handles.id}.expTableXLS,'parameters',pars);
        ...
        CT{handles.id}.eTable = dataOut.eTable;
...
elseif strcmp(currStepName,'Save Aligned Data') % 'Adjust Alignment'
    ...
    [CT{handles.id}.fidMapData,CT{handles.id}.datMapData,...
     CT{handles.id}.fidFrames,CT{handles.id}.dataFrames] = ...
        ChrTracer3_SaveAlignedData(CT{handles.id}.eTable,CT{handles.id}.regData,'parameters',pars);
...
elseif strcmp(currStepName,'Fit Spots') 
    ...
    pars.numParallel = 1;
    pars.eTable = CT{handles.id}.eTable;  % 
    % spotTableTemp = ChrTracer3_FitAllSpots(CT{handles.id}.fidSpots,CT{handles.id}.dataSpots,'parameters',pars,'saveTable',false,'veryverbose',true); % 
    spotTableTemp = ChrTracer3_FitSpots(CT{handles.id}.fidSpots{s},CT{handles.id}.dataSpots{s},s,'parameters',pars,'veryverbose',true);
...
if exist(fovSaveFolder,'dir')~=7  && CT{handles.id}.parsFit.saveData
    mkdir(fovSaveFolder);
end
% -------------------------- CLEAR UP -----------------------------------% 
...
[datFolder,expXLS] = fileparts(CT{handles.id}.expTableXLS);
savedXLS = [saveFolder,expXLS,'.xlsx'];
if ~exist(savedXLS,'file')
   copyfile( CT{handles.id}.expTableXLS,savedXLS);
end
...
[CT{handles.id}.fidMapData,CT{handles.id}.datMapData,...
 CT{handles.id}.fidFrames,CT{handles.id}.dataFrames] = ...
 ChrTracer3_SaveAlignedData(CT{handles.id}.eTable,CT{handles.id}.regData,...
    'parameters',pars,...
...
pars.showPlots = false; % don't show plots when running in batch
pars.selectSpots = 1:size(CT{handles.id}.parsFOV.lociXY,1);
spotDataTable = ChrTracer3_FitAllSpots(CT{handles.id}.fidSpots,CT{handles.id}.dataSpots,...
    'parameters',pars,'saveTable',false,'eTable',CT{handles.id}.eTable);
...
daxFiles1 = cellstr(ls([saveFolder,'f*h001_fid.dax']));   
sourceFolder = fileparts(get(handles.EditTextFolder,'String'));
daxFiles2 = cellstr(ls([sourceFolder, filesep,CT{handles.id}.eTable.FolderName{1},filesep,'C*.dax']));
...
% determine number of FOV in data folder
sourceFolder = fileparts(get(handles.EditTextFolder,'String'));
rawDax = cellstr(ls([sourceFolder, filesep,CT{handles.id}.eTable.FolderName{1},filesep,'C*.dax']));
...
%================== Auto Register ===============  NOT FUNCTIONAL YET ========
...
spotDataTable = ChrTracer3_FitAllSpots(CT{handles.id}.fidSpots,CT{handles.id}.dataSpots,'parameters',pars,'saveTable',false);
tableSaveName = [CT{handles.id}.parsFOV.saveFolder, 'fov',num2str(pars.fov,'%03d'),'_AllFits.csv'];
writetable(spotDataTable,tableSaveName);
disp(['wrote ',tableSaveName]);
%-------------------------------- SAVE DATA ------------------------------% 
