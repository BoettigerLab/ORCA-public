function [spotTrace1,spotTrace2,spotsPerStep] = LinkXYpairs(fits1,fits2c,xy1p,xy2p,zDepth,varargin)
%% Method
...
%% optional pars
defaults = cell(0,3); % 
% shared parameters for mvoing average
...
defaults(end+1,:) = {'movAveStepMaxFold','positive',2.5}; % max fold change in step size relative to local average, used by Remove Jumps
...
defaults(end+1,:) = {'maxDistToSeed','positive',20}; % maximum distance to seed point to be counted in the coarse linking
% parameters for refined measurement of moving average
defaults(end+1,:) = {'windowR','positive',30}; % 'radius' of window around the original seed point in which valid localizations may occur
...
defaults(end+1,:) = {'minObsPerAve2','positive',5}; % minimum number of observations in the moving average window to be counted as a valid obs
...
defaults(end+1,:) = {'maxDistFromMovingAve','positive',3}; % Max distance an acceptable spot can be from its local-time-moving average centroid reported as fold-change relative to moving-average stepsize
defaults(end+1,:) = {'maxStep','positive',6}; % distance in pixels, the max distance from the last localization to be added to the trace (permisive)   
...
defaults(end+1,:) = {'movAveFig','integer',12}; % per trace
defaults(end+1,:) = {'spotMapFig','integer',12}; % per trace
defaults(end+1,:) = {'traceFig','integer',14}; %  per trace
...
defaults(end+1,:) = {'parallel','boolean',false}; % NOT YET WRITTEN
...
for n=1:nSpots
    moveAveAllInt1(n,:,1:2) = RemoveJumps(squeeze(moveAveAllInt1(n,:,1:2)),'maxDiff',pars.movAveStepMaxFold,'localRegion',pars.movAveLocalStep);
    moveAveAllInt2(n,:,1:2) = RemoveJumps(squeeze(moveAveAllInt2(n,:,1:2)),'maxDiff',pars.movAveStepMaxFold,'localRegion',pars.movAveLocalStep);
    moveAveAllInt1(n,:,1) = fillmissing(squeeze(moveAveAllInt1(n,:,1)),'movmean',w);
    moveAveAllInt1(n,:,2) = fillmissing(squeeze(moveAveAllInt1(n,:,2)),'movmean',w);
    moveAveAllInt2(n,:,1) = fillmissing(squeeze(moveAveAllInt2(n,:,1)),'movmean',w);
    moveAveAllInt2(n,:,2) = fillmissing(squeeze(moveAveAllInt2(n,:,2)),'movmean',w);
end

if pars.figCoarseMovAve > 0
    figure(pars.figCoarseMovAve); clf; Ncolor(tLapseMov); hold on; 
    ts = 1:tStps;
    for n=1:nSpots % n=75
        scatter(squeeze(moveAveAllInt1(n,:,1)),squeeze(moveAveAllInt1(n,:,2)),[],ts','filled');
...
if ~pars.parallel
    for s = pars.selSpots % s=8
                ...
                for t=z:zDepth:nFrames 
                    ...
                    if ~isnan(xy1)
                        ...
                        if ~isempty(fr1)
                            ...
                            [minDistLast,idMinDist] = min(distLast);
                            if minDistLast < maxStep1 && distMov(idMinDist) < maxDistFromMovAve1
                                ...
                                spotTrace1(s,t,1:2) = refPt1;
                                spotTrace1(s,t,5:end) = curDat1(idMinDist,:);
                    ...
                    if ~isnan(xy2)
                        ...
                        if ~isempty(fr2)
                            ...
                            %  the moving average.  
                            [minDistLast,idMinDist] = min(distLast);
                            if minDistLast < maxStep2 && distMov(idMinDist) < maxDistFromMovAve2
                                spotsPerStep(s,t,2) = size(newPts2,1);
                                refPt2 =  newPts2(idMinDist,:);
                                spotTrace2(s,t,1:2) = refPt2;
                                spotTrace2(s,t,5:end) = curDat2(idMinDist,:);
        ...
        if pars.movAveFig > 0
        figure(pars.movAveFig); hold on;
            for z=1:zDepth
                xy = squeeze(spotTrace1(s,z:zDepth:end,1:2));
                plot(xy(:,1),xy(:,2),'.','color',[.5 0 0],'MarkerSize',2);
                xy = squeeze(spotTrace2(s,z:zDepth:end,1:2));
        ...
        npp=pars.npp;
        if pars.traceFig > 0
            figure(pars.traceFig); clf;
            for z=1:zDepth
                subplot(zDepth,1,z);
                tim = z:zDepth:nFrames;
                d12 = abs(spotTrace1(s,z:zDepth:end,1) - spotTrace2(s,z:zDepth:end,1));
                dd12 = nanmedian(abs(diff(d12(~isnan(d12)))));
                x = squeeze(spotTrace1(s,z:zDepth:end,1));
                plot(tim(~isnan(x)),x(~isnan(x)),'.-'); hold on;
                dx1 = npp*nanmedian(abs(diff(x(~isnan(x)))));
                nx1 = sum(~isnan(x));
                x = squeeze(spotTrace2(s,z:zDepth:end,1));
                plot(tim(~isnan(x)),x(~isnan(x)),'.-'); hold on;
                dx2 = npp*nanmedian(abs(diff(x(~isnan(x)))));
                nx2 = sum(~isnan(x));
                title(['d: ',num2str(npp*nanmedian(d12),3), 'nm.  \delta d:',num2str(npp*dd12,3),...
...
%% parallel
    ...
    parfor s = pars.selSpots  % parfor
        try
            sFit1 = sFits1{s};
            sFit2 = sFits2{s};
            spotTraces1{s} = nan(nFrames,8);
            spotTraces2{s} = nan(nFrames,8);
            spotsPerSteps{s} = nan(nFrames,2);
            movAve1 = nan(tStps,2);
            movAve2 = nan(tStps,2); 
            for t=1:tStps % t=9
                % to avoid the occassional stray-background point in an early frame
            ...
            hasDat1 = ~isnan(moveAveClean1(:,1));
            aveStep1 = sqrt(sum(diff(moveAveClean1(hasDat1,:)).^2,2));
            medAveStep1 = nanmedian(aveStep1);
            moveAveClean2 = RemoveJumps(movAve2,'maxDiff',pars.movAveStepMaxFold,'localRegion',pars.movAveLocalStep);
            hasDat2 = ~isnan(moveAveClean2(:,1));
            aveStep2 = sqrt(sum(diff(moveAveClean2(hasDat2,:)).^2,2));
            medAveStep2 = nanmedian(aveStep2); 
                    ...
                    if ~isnan(xy1)
                        ...
                        if ~isempty(fr1)
                            ...
                            if minDistLast < maxStep1 && distMov(idMinDist) < maxDistFromMovAve1
                                ...
                                refPt1 =  newPts(idMinDist,:);
                                spotTraces1{s}(t,1:2) = refPt1;
                                spotTraces1{s}(t,5:end) = curDat1(idMinDist,:);
                                maxStep1 = pars.maxStep;
                            end
                        end   
                    end
                    if ~isnan(xy2)
                        ...
                        if ~isempty(fr2)
                            ...
                            if minDistLast < maxStep2 && distMov(idMinDist) < maxDistFromMovAve2
                                ...
                                refPt2 =  newPts2(idMinDist,:);
                                spotTraces2{s}(t,1:2) = refPt2;
                                spotTraces2{s}(t,5:end) = curDat2(idMinDist,:);
                                maxStep2 = pars.maxStep;
                            end
                        end
                    end
    ...
    try
        spotTrace1 = cat(3,spotTraces1{:});
        spotTrace1 = permute(spotTrace1,[3,1,2]); % swap dims 
        spotTrace2 = cat(3,spotTraces2{:});
        spotTrace2 = permute(spotTrace2,[3,1,2]); % swap dims 
