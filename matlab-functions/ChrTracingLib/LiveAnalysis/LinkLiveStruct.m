function [pntArray,idx,pntChain] = LinkLiveStruct(fitStruct,varargin)
...
defaults(end+1,:) = {'uniqueMatch','boolean','true'};  % the knn option is not robust to varying numbers 
defaults(end+1,:) = {'symbol','string','.'}; 
defaults(end+1,:) = {'figHandle','freeType',[]}; % 
defaults(end+1,:) = {'indexByFrame','boolean',true}; % 
...
if pars.indexByFrame
    if istable(fitStruct)
        frameNames = fitStruct.frame;
        fTot = max(frameNames);
        xs = cell(fTot,1);
        ys = cell(fTot,1);
        frm= cell(fTot,1);
        ...
        sig= cell(fTot,1);
        for f=1:fTot
            xs{f} = fitStruct.x(fitStruct.frame==f);
            ys{f} = fitStruct.y(fitStruct.frame==f);
            frm{f} = fitStruct.frame(fitStruct.frame==f);
            hs{f} = fitStruct.height(fitStruct.frame==f);
            bkd{f} =fitStruct.background(fitStruct.frame==f);
            sig{f} =fitStruct.xsigma(fitStruct.frame==f);
        end
    ...
    % don't index by frame
    if istable(fitStruct)
        frameNames = unique(fitStruct.frame);
        fTot = length(frameNames);
        xs = cell(fTot,1);
        ys = cell(fTot,1);
        frm= cell(fTot,1);
        hs = cell(fTot,1);
        ...
        sig= cell(fTot,1);
        for f=1:fTot
            xs{f} = fitStruct.x(fitStruct.frame==frameNames(f));
            ys{f} = fitStruct.y(fitStruct.frame==frameNames(f));
            frm{f} = fitStruct.frame(fitStruct.frame==frameNames(f));
            hs{f} = fitStruct.height(fitStruct.frame==frameNames(f));
            bkd{f} =fitStruct.background(fitStruct.frame==frameNames(f));
            sig{f} =fitStruct.xsigma(fitStruct.frame==frameNames(f));
        end
    elseif isstruct(fitStruct)
        xs = {fitStruct.x};
        ys = {fitStruct.y};
...
for f=1:nFrames % -1
    ...
    if showPlot && f>2 && pars.figHandle >0
...
if isstruct(fitStruct)
    for f=1:nFrames  % f = 5
        ...
        if ~isempty(m)
            ...
            % z % too be computed later, leave nans for now
            ...
            pntArray(m(:,2),f,7) = fitStruct(f).sigma(m(:,1)); % spot width  (can use this later for calculating z)
        end
    end
else
    for f=1:nFrames  % f = 5
        m = idx{f};
        if ~isempty(m)
            pntArray(m(:,2),f,1) = xs{f}(m(:,1)); %   fitStruct(f).x(m(:,1)); % that's compact but not super legible;  
            pntArray(m(:,2),f,2) = ys{f}(m(:,1)); % fitStruct(f).y(m(:,1)); % the pattern is helping with the reading
            % z % too be computed later, leave nans for now
            pntArray(m(:,2),f,4) = frm{f}(m(:,1));  %  fitStruct(f).frame*ones(length(m(:,1)),1); % time (by frame)
            pntArray(m(:,2),f,5) = hs{f}(m(:,1)); %  fitStruct(f).height(m(:,1)); % spot brightness  (will need this later for calculating z)
            pntArray(m(:,2),f,6) = bkd{f}(m(:,1));  % fitStruct(f).background(m(:,1)); % spot brightness  (will need this later for calculating z)
            pntArray(m(:,2),f,7) = sig{f}(m(:,1)); % fitStruct(f).sigma(m(:,1)); % spot brightness  (can use this later for calculating z)
        end
    end
end
...
t_out = toc;
if pars.verbose
    disp(['all points linked in ',num2str(t_out),' seconds']);
end