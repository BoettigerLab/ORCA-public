function fitsO = DaoFitZ_FromTable(fits1,varargin)
% inputs 
%   fits1 - a table from LoadHD5Fits (using csv load)
% 
% outputs
%  fitsO - a Table with fields: x,y,z,xm,ym,h,zw,za
...
% 
%  Notes
% 2025-04-29 - adapted from DaoFitZ, rewritten to accept table input
% instead of MoleculeListStructure.  Note, table2struct does not build
% molecule lists from tables. 
% should write Table2Mlist  and a Mlist2Table function to help use the
% STORM data stuff (we have something of the sort embedded in LoadHD5Fits)
...
% if istable(fits1)
%     fitTable = table2struct(fits1);
% end

xs = fits1.x;
ys = fits1.y;
if isempty(pars.seedLoci)
    [xy0,~,cnts] = TraceCenters({xs},{ys},...
        'minSpotsPerBin',pars.minSpotsPerTrace,...
        'minSpotsPerTrace',pars.minSpotsPerTrace, ...
        'binResolution',pars.seedBinResolution,'showPlots',pars.showSeed);
    % note that TraceCenters still expects cell arrays of xs and ys, 1
    % cell entry per frame with multiple spots per frame.
else
    xy0 = pars.seedLoci;
end

% % a nearest-frame approximation (this will double/triple count stacks)
if pars.minSpotsPerTrace == 1
zs = fits1.frame;
[m,dis] = knnsearch([xs,ys],xy0);
xyz = [xy0,zs(m)];
end
...
for f=1:nFrames  % f = 5
    m = idx{f};
    if ~isempty(m)
        currFrameTable = fits1(fits1.frame==selFrames(f),:);  
        pntArray(m(:,2),f,1) = currFrameTable.x(m(:,1)); %  
        pntArray(m(:,2),f,2) = currFrameTable.y(m(:,1)); % 
        pntArray(m(:,2),f,5) = currFrameTable.height(m(:,1)); % spot brightness  (will need this later for calculating z)
        pntArray(m(:,2),f,4) = currFrameTable.frame(m(:,1)); %  % time (by frame)  - frame can be a good proxy for z.  
        pntArray(m(:,2),f,6) = currFrameTable.background(m(:,1)); % spot brightness  (will need this later for calculating z)
    end
end
