...
% This class designs target regions given a transcriptome. 
%--------------------------------------------------------------------------
% reverted old class. New version broke this.
% manually added other bug fixes.
% 
% Jeffrey Moffitt
...
properties (SetAccess=protected)
    ...
    dG                      % The free energies associated with all nearest neighbors
    ...
    specificityTable        % The OTTable used to calculate region specificity denominator
    selfTables              % The OTTable used to calculate region specificity numerator
end
...
methods
    ...
    function obj = TRDesigner(varargin)
        ...
        % -------------------------------------------------------------------------
        if ~isempty(parameters.specificityTable)
            obj.AddSpecificityTable(parameters.specificityTable,parameters.selfTables);
        end
    ...
    function AddForbiddenSeq(obj, seq, varargin)
        ...
        if obj.verbose
            PageBreak();
            tic;
            display(['Finding all occurences of ' int2nt(seq+1)]);
        end
        ...
        forbiddenHash = sum(hashBase.*seq) + 1;
        seqs = obj.transcriptome.intSequences; 

        parfor (s=1:length(seqs), obj.numPar) % Loop over transcriptome
            % Hash sequence
            seqHash = filter(hashBase, 1, seqs{s});
    ...
    function AddOTTable(obj, otTable, tableName, varargin)
        ...
        parfor (s=1:length(seqs), obj.numPar) 
    ...
    % -------------------------------------------------------------------------
    function AddSpecificityTable(obj, specificityTable, varargin)
        % Add or replace an existing penalty table
        % obj.AddSpecificityTable(obj, specificityTable)
        ...
        % -------------------------------------------------------------------------
%         defaults = cell(0,3);
%         defaults(1,:) = {'selfTables', 'cell', {}}; % Either add to existing tables (false) or replace (true)
%         pars = ParseVariableArguments(varargin, defaults, mfilename);
        if nargin == 3
           selfTables = varargin{1};
        else
            selfTables = {};
        end
        ...
        % Make reference to table
        table = obj.specificityTable; % [Q: is this not passed as a var?]
        
        % Determine normalization based on the weighting of the table
        if obj.specificityTable.uniformWeight 
            normalization = ones(1, length(seqs));
        else
            normalization = obj.transcriptome.abundance;
        end
        
        % Loop over all sequences
        for s=1:length(seqs) % While useful, parfor can raise serious memory issues here
            if ~isempty(selfTables)
                norm = selfTables{s}.CalculatePenalty(seqs{s});
            else
                norm = normalization(s);
            end
            specificity{s} = norm./(table.CalculatePenalty(seqs{s}));
    ...
    function [noForbiddenSeqs, ids, names] = GetRegionForbiddenSeqs(obj, len, varargin)
        ...
        for s=1:length(obj.forbiddenSeqs) % Loop over forbidden sequences
            % Display progress
            if obj.verbose
                display(['Finding ' obj.forbiddenSeqs{s}]);
            end
            ...
            if s==1
                for l=1:length(inds) % Loop over transcriptome
                    hasSeq = filter(ones(1, windowLen)/windowLen, 1, obj.hasForbiddenSeqs{s}{inds(l)});
    ...
    % -------------------------------------------------------------------------
    function [targetRegions,indsToKeep] = DesignTargetRegions(obj,varargin)
        ...
        defaults(end+1,:) = {'threePrimeSpace', 'float', 0};
        defaults(end+1,:) = {'removeForbiddenSeqs', 'boolean', false};
        defaults(end+1,:) = {'noOverlap', 'boolean', true};
        ...
        for l=1:length(parameters.regionLength)
            ...
            if parameters.removeForbiddenSeqs
                noForbiddenSeqs = GetRegionForbiddenSeqs(parameters.regionLength(l), 'inds', inds);
        ...
        parfor (i=1:length(inds), obj.numPar)   
            ...
            regionProps = zeros(5,0);
            for l=1:length(regionLength)
                startPos = find(indsToKeep{l,i});
                localProps = zeros(5,length(startPos));
            ...
            if noOverlap
                ...
                selectedRegionProps = regionProps;
            end
...
methods (Static)
    ...
    function dG = SantaLuciaNearestNeighbor(intSeq)
        ...
        % -------------------------------------------------------------------------
        % Nearest neighbor H and S from Santa Lucia Jr, PNAS 95, 1460-65
        % (1998)
        % -------------------------------------------------------------------------
