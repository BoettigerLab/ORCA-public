...
if ~(exist(tableSaveName,'file')==2 && exist(imageSaveName,'file')==2) || pars.overwrite
    ...
    if ~isempty(pars.eTable)
        ...
        bitNum = 1:numHybes;
        dataType = cellstr(repmat('H',numHybes,1));
    end
    ...
    if pars.showPlots
        ...
        stk0xz = cat(3,stk{:});
        subplot(2,2,1);  Ncolor(3/numHybes*IncreaseContrast(stk0xy)); title('corr. fid x,y');
        hold on; plot(fidTable.x,fidTable.y,'r+');
        subplot(2,2,2);  Ncolor(3/numHybes*IncreaseContrast(stk0xz)); title('corr. fid x,z');
    ...
    if ~isempty(fidTable.x)
        for h=1:numHybes % h=1
            ...
            for i=1:nFits
                ...
                dTable.read = bitNum(h,n)*ones(length(dTable.x),1);
                ...
                dTable.chn = n*ones(length(dTable.x),1);
        ...
        if pars.showPlots 
            ...
            stk1xz = cat(3,stk{:});
            subplot(2,2,3);  Ncolor(2/numHybes*IncreaseContrast(stk1xy)); title('corr. fid x,y');
            hold on; plot(fidTable.x,fidTable.y,'r+');
            subplot(2,2,4); Ncolor(2/numHybes*IncreaseContrast(stk1xz)); title('corr. fid x,z');
            hold on; plot(fidTable.x,fidTable.z,'r+');
